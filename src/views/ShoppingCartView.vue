<template>
  <main class="cart-container">
          ...
          <tr v-for="item in cart" :key="item.id" class="cart-item-row">
            ...
            <td>
              <div class="quantity-controls">
                ...
                <input type="number" class="quantity-display" :value="item.quantity" min="1" max="3" @change="updateQuantity(item.id, parseInt($event.target.value) || 1)">
                <button class="quantity-btn" @click="updateQuantity(item.id, item.quantity + 1)">
    ...
    <div v-if="cart.length > 0" class="cart-summary">
      ...
      <small v-if="shipping === 0" class="text-success d-block mb-2"><i class="fas fa-gift"></i> Bạn được miễn phí vận chuyển!</small>
      <div class="summary-row">
        <span>Tổng cộng:</span>
        ...
        <div class="checkout-modal-content">
            ...
            <div class="checkout-modal-body">
                <div class="payment-methods">
                    <div class="payment-option" :class="{'selected': selectedPaymentMethod === 'bank'}" @click="selectPaymentMethod('bank')">
                     ...
                     <div class="payment-option" :class="{'selected': selectedPaymentMethod === 'momo'}" @click="selectPaymentMethod('momo')">
                    ...
                    <div class="payment-option" :class="{'selected': selectedPaymentMethod === 'zalopay'}" @click="selectPaymentMethod('zalopay')">
                     ...
                     <div class="payment-option" :class="{'selected': selectedPaymentMethod === 'cod'}" @click="selectPaymentMethod('cod')">
                    ...
                    <div class="checkout-totals">
                        ...
                        <div class="total-row final-total">
                            <div v-if="requiresDeposit" style="width: 100%; text-align: left;">
                                ...
                                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem; color: #de0000;">
                                    <span>Tiền đặt cọc (10%):</span>
                                    <span style="text-align: right;">{{ formatCurrency(depositAmount) }}</span>
                                </div>
                                <div style="display: flex; justify-content: space-between;">
                                    <span>Tiền còn lại:</span>
                                    <span style="text-align: right;">{{ formatCurrency(checkoutTotal - depositAmount) }}</span>
                                </div>
                            </div>
...
export default {
  ...
  methods: {
    ...
    loadCartFromCookie() {
        try {
            ...
            if (this.cart.length === 0) {
                 this.cart = [
                    { id: 'cbr150r', name: 'Honda CBR150R', price: 72000000, quantity: 1, image: 'https://placehold.co/80x80/de0000/ffffff?text=CBR150R' },
                    { id: 'vario160', name: 'Honda Vario 160', price: 55000000, quantity: 2, image: 'https://placehold.co/80x80/333333/ffffff?text=Vario160' }
                 ];
            }
        } catch (error) {
    ...
    updateQuantity(productId, newQuantity) {
      ...
      if (quantity > 3) {
        alert('Số lượng mỗi sản phẩm chỉ có thể mua tối đa là 3. Để mua số lượng lớn hơn, vui lòng liên hệ trực tiếp tại cửa hàng.');
        ...
        switch (this.selectedPaymentMethod) {
            ...
            case 'momo':
             ...
             case 'zalopay':
                ...
                break;
            case 'cod':
                const codAmount = this.requiresDeposit ? (totalWithFees - this.depositAmount) : totalWithFees;
                paymentInfo = `
