<template>
    <div id="product-detail-panel" :class="{ open: isOpen }">
        <div class="flex items-center justify-between p-6 border-b bg-gradient-to-r from-honda-red to-red-700">
            ...
            <button @click="$emit('close')" class="text-white text-2xl hover:bg-white hover:bg-opacity-20 rounded-full p-2 transition-all">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <div class="p-6">
            <div class="text-center mb-6">
                <img :src="product.image" :alt="product.name" class="w-full h-80 object-contain mb-4 bg-gray-100 rounded-xl">
            </div>
            <div class="text-center mb-6">
            ...
            <div class="flex items-center justify-center mb-6">
                ...
                <div class="flex items-center border-2 border-gray-300 rounded-lg">
                    <button class="px-3 py-1 hover:bg-gray-100" @click="quantity > 1 ? quantity-- : 1">-</button>
                    <input type="number" v-model.number="quantity" min="1" class="w-16 text-center border-none outline-none">
                    <button class="px-3 py-1 hover:bg-gray-100" @click="quantity++">+</button>
                </div>
            </div>

            <button @click="$emit('addToCart', quantity)" class="bg-gradient-to-r from-honda-red to-red-700 text-white w-full py-4 rounded-xl text-lg font-semibold hover:from-red-700 hover:to-red-800 transition duration-300 transform hover:scale-105">
