<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
    <div v-if="product" class="bg-white rounded-lg shadow-lg max-w-3xl w-full overflow-auto">
      <div class="flex justify-end p-3">
        <button class="text-2xl text-gray-600 hover:text-gray-900" @click="emit('close')">
          &times;
        </button>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
        ...
        <div>
          ...
          <h4 class="text-xl text-red-600 font-bold mb-4">{{ formatPrice(product.price) }}</h4>
