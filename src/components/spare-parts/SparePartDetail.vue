  ...
  <transition name="fade" mode="out-in">
    <div v-if="product" :key="product.id" class="container mx-auto px-4 py-8 mt-4 min-h-screen">
      <nav class="flex text-sm text-gray-500 mb-6 gap-2 items-center">
        ...
        <router-link to="/spare-parts" class="hover:text-red-600 transition-colors">Phụ tùng</router-link>
        <i class="fas fa-chevron-right text-[10px]"></i>
      ...
      <div class="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 mb-16">
        <div class="flex flex-col lg:flex-row gap-12">
          <div class="w-full lg:w-1/2">
            <div class="bg-gray-50 rounded-2xl p-6 md:p-12 flex items-center justify-center border border-gray-50 sticky top-24">
              <img 
                :src="product.image" 
                :alt="product.name" 
                class="max-w-full h-auto object-contain hover:scale-105 transition-transform duration-500" 
              />
            </div>
          </div>
          ...
          <div class="w-full lg:w-1/2 flex flex-col">
            <div class="mb-6">
              <span class="inline-block px-3 py-1 bg-red-50 text-[#de0000] text-xs font-bold rounded-full mb-4 uppercase tracking-widest">
                {{ product.type }}
              </span>
            ...
            <div class="space-y-8 flex-grow">
              <div class="pt-6 border-t border-gray-100">
                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-3">Mô tả phụ tùng</h3>
                <p class="text-gray-600 leading-relaxed text-lg">
                  {{ product.description || 'Thông tin kỹ thuật đang được cập nhật cho loại phụ tùng này.' }}
                </p>
              </div>

              <div class="pt-6 border-t border-gray-100">
                <h3 class="text-sm font-bold text-gray-400 uppercase tracking-widest mb-3">Số lượng đặt hàng</h3>
                <div class="flex items-center gap-6">
                  <div class="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden shadow-sm bg-white">
                    ...
                    <input type="number" v-model.number="quantity" class="w-14 text-center font-bold text-lg focus:outline-none pointer-events-none" min="1" />
              ...
              <div class="pt-8">
                ...
                >
                  <i class="fas fa-tools"></i> THÊM VÀO GIỎ HÀNG
                </button>
              </div>
            </div>
          </div>
      ...
      <div class="mt-20">
        <div class="flex items-center justify-between mb-8 border-b border-gray-200 pb-5">
          <div class="flex items-center gap-3">
            ...
            <h2 class="text-2xl font-bold text-gray-800 uppercase tracking-tight">Có thể bạn cần</h2>
          </div>
          <router-link to="/spare-parts" class="text-[#de0000] font-bold hover:text-black transition-colors flex items-center gap-2 group">
            Xem thêm <i class="fas fa-arrow-right text-sm group-hover:translate-x-1 transition-transform"></i>
          </router-link>
        </div>
        
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
          <ProductCard
            v-for="item in relatedSpareParts"
            :key="item.id"
            :product="item"
    ...
    <div v-else class="flex flex-col justify-center items-center h-[80vh] text-gray-400 gap-4">
      ...
      <div class="text-lg font-medium animate-pulse">Đang tải phụ tùng...</div>
    </div>
  </transition>
</template>
...
/**
 * FIX: Lỗi không hiển thị trang chi tiết
 * Sử dụng String() để đảm bảo so sánh ID chính xác tuyệt đối
 */
const loadData = () => {
  ...
  if (foundProduct) {
    ...
    quantity.value = 1
    // Cuộn lên đầu trang mượt mà khi chuyển đổi sản phẩm liên quan
    window.scrollTo({ top: 0, behavior: 'smooth' })
  } else {
    // Nếu ID không tồn tại trong data, đẩy về trang danh sách để tránh lỗi trắng trang
    router.replace({ name: 'spare-parts' })
  }
}

/**
 * Lọc sản phẩm liên quan: Cùng loại nhưng khác ID hiện tại
 */
const relatedSpareParts = computed(() => {
...
/**
 * Chuyển hướng sang chi tiết phụ tùng khác an toàn
 */
const goToRelatedDetail = (p) => {
  if (p?.id) {
    router.push({ name: 'spare-part-detail', params: { id: String(p.id) } })
  }
}
...
/**
 * Watch route ID: Cực kỳ quan trọng để cập nhật trang khi người dùng 
 * nhấn vào sản phẩm tương tự mà không làm load lại toàn bộ website
 */
...
/* Ổn định scrollbar để tránh nhảy layout (Jitter) */
.container {
  scrollbar-gutter: stable;
}

/* Ẩn nút mũi tên của input number để UI sạch sẽ hơn */
