    ...
    <div class="invoice-modal" :class="{ show: props.show }">
      <div id="invoice-printable" v-if="props.orderData" class="invoice-content">
        ...
        <div class="invoice-body">
          <div class="mb-6">
            ...
            <div class="flex justify-between border-b py-1">
              <strong>Tên cửa hàng:</strong><span>AnhEm Motor</span>
            </div>
            <div class="flex justify-between border-b py-1">
              <strong>Địa chỉ:</strong><span>193 Đỗ Văn Thi, Trảng Biên, Biên Hòa, Đồng Nai</span>
            </div>
            <div class="flex justify-between py-1">
              <strong>Điện thoại:</strong><span>012 3456 7890</span>
            </div>
          </div>
          ...
          <table class="w-full border-collapse mb-4">
            ...
            <tbody>
              <tr
                v-for="item in props.orderData.cart"
                :key="item.id"
          ...
          <div class="space-y-2 border-t pt-3">
            <div class="flex justify-between">
              ...
              ><span>{{ formatCurrency(props.orderData.totals.subtotal) }}</span>
            </div>
            <div class="flex justify-between">
              <span>Phí vận chuyển:</span
              ><span>{{
                props.orderData.totals.shipping === 0
                  ? 'Miễn phí'
                  : formatCurrency(props.orderData.totals.shipping)
              }}</span>
            </div>
            <div v-if="props.orderData.totals.codFee > 0" class="flex justify-between">
              <span>Phí COD:</span><span>{{ formatCurrency(props.orderData.totals.codFee) }}</span>
            </div>
            <div class="flex justify-between">
            ...
            <template v-if="props.orderData.totals.requiresDeposit">
              <div class="flex justify-between pt-2 text-red-600 font-semibold">
                <span>Tổng giá trị:</span
                ><span>{{ formatCurrency(props.orderData.totals.total) }}</span>
              </div>
              <div class="flex justify-between text-red-600 font-semibold">
                <span>Tiền đặt cọc (10%):</span
                ><span>{{ formatCurrency(props.orderData.totals.depositAmount) }}</span>
              </div>
              <div class="flex justify-between pt-2 font-semibold">
                <span>Tiền còn lại:</span
                ><span>{{
                  formatCurrency(
                    props.orderData.totals.total - props.orderData.totals.depositAmount,
                  )
                }}</span>
              </div>
            </template>
        ...
        <div class="flex justify-center gap-4 mt-6">
          <button
            class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-full"
            @click="printInvoice"
          >
            <i class="fas fa-print mr-2"></i>In hóa đơn
          </button>
          <button
            class="border border-gray-300 px-5 py-2 rounded-full text-gray-700"
            @click="close"
          >
            Đóng
          </button>
